...
class 對齊合成(腳本程式):
	def 訓練合成模型(self, 音檔目錄, 拼音句檔名, 標仔拼音,
		...
		'共拼音句轉做HTK愛的樣'
		_拼音句轉標仔檔 = 拼音句轉標仔檔()
		原始標仔資料夾 = self.細項目錄(這馬目錄, '原始標仔')
		完整標仔夾, 主要標仔夾 = _拼音句轉標仔檔.轉(標仔拼音, 拼音句檔名, 原始標仔資料夾)
		
		'用HTK標語料的時間'
		_對齊音檔時間 = 對齊音檔時間()
		辨識資料 = self.細項目錄(這馬目錄, '辨識資料')
		音節聲韻對照檔 = os.path.join(這馬目錄, '音節聲韻對照檔.dict')
		self.陣列寫入檔案(音節聲韻對照檔, 文本音值表.聲韻對照())
		對齊的主要音值資料夾 = _對齊音檔時間.對齊(辨識資料, 音檔目錄, 主要標仔夾, 音節聲韻對照檔, HTK執行檔路徑)
		
		'準備HTS需要的物件'
		HTS資料目錄 = self.細項目錄(這馬目錄, 'data')
		'	1.HTS愛標仔'
